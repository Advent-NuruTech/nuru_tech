    ...
    <div className="min-h-screen overflow-x-hidden">
      ...
      <section className="mx-auto mt-14 max-w-6xl">
        <div className="mb-6 flex items-center justify-between">
          <SectionHeading className="mb-0">Portfolio Showcase</SectionHeading>
          <Link href="/portfolio" className="text-sm font-semibold text-cyan-700 hover:underline dark:text-cyan-300">
            View All
          </Link>
        </div>
        <div className="marquee-container relative flex overflow-x-hidden">
          <div className="marquee-content flex min-w-full shrink-0 animate-marquee items-stretch gap-5 hover:[animation-play-state:paused]">
            {featuredPortfolio.map((item, index) => (
              (() => {
                const links = getPortfolioLinks(item);
                return (
              <article
                key={`portfolio-flow-1-${item.id}-${index}`}
                className="w-[min(88vw,24rem)] shrink-0 overflow-hidden rounded-3xl border border-neutral-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl dark:border-neutral-700 dark:bg-neutral-900"
              >
          ...
          <div
            aria-hidden="true"
            className="marquee-content flex min-w-full shrink-0 animate-marquee items-stretch gap-5 hover:[animation-play-state:paused]"
          >
            {featuredPortfolio.map((item, index) => (
              (() => {
                const links = getPortfolioLinks(item);
                return (
              <article
                key={`portfolio-flow-2-${item.id}-${index}`}
                className="w-[min(88vw,24rem)] shrink-0 overflow-hidden rounded-3xl border border-neutral-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl dark:border-neutral-700 dark:bg-neutral-900"
              >
                <div className="relative h-52 w-full bg-neutral-100 dark:bg-neutral-800">
                  <Image
